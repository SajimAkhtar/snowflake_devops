CREATE OR REPLACE PROCEDURE "DW_DIM_INCR"("PROCESSID" FLOAT, "TABLE_NAME" VARCHAR(16777216), "SCDTYPE" VARCHAR(16777216), "LOAD_TYPE" VARCHAR(16777216))
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
...
try {
 ...
 TARGET_TABLE) values (?,''SNOWFLAKE'',?,''SQLSERVER'',''POLICYCENTER_DB'',''1'',''CLOUD'',?,?,''SNOWFLAKE'',
	...
	var v_stmt = `select sql_query from ETL.METADATA_IA where TABLE_NAME = ? AND LOAD_TYPE = ? and SCD_TYPE = ?;`
	var sf_stmt = snowflake.createStatement({sqlText: v_stmt, binds : [TABLE_NAME,LOAD_TYPE, SCDTYPE]});
	...
	var ins_success_stmt = `update ETL.dw_control_table set JOB_STATUS =''SUCCESS'' where processid = ? and TARGET_TABLE = ?;`
	var sf_ins_success_stmt = snowflake.createStatement({sqlText: ins_success_stmt,binds : [PROCESSID,TABLE_NAME]});
