CREATE OR REPLACE PROCEDURE "DW_FACT_AGG"("PROCESSID" FLOAT, "TABLE_NAME" VARCHAR(16777216), "AGGREGATE_LEVEL" VARCHAR(16777216), "LOAD_TYPE" VARCHAR(16777216))
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
...
try {
 ...
 TARGET_TABLE) values (?,''SNOWFLAKE'',?,''SQLSERVER'',''POLICYCENTER_DB'',''1'',''CLOUD'',''INSURANCE_ANALYTICS'',''PCDM'',''SNOWFLAKE'',
    ''DAILY'', CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,''STARTED'',?);`
	var sf_ins_stmt = snowflake.createStatement({sqlText: ins_stmt,binds : [PROCESSID,jobname,tblname]});
