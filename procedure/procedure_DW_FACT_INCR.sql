CREATE OR REPLACE PROCEDURE "DW_FACT_INCR"("PROCESSID" FLOAT, "TABLE_NAME" VARCHAR(16777216), "SCD_TYPE" VARCHAR(16777216), "LOAD_TYPE" VARCHAR(16777216))
...
try {
 ...
 TARGET_TABLE) values (?,''SNOWFLAKE'',?,''SQLSERVER'',''POLICYCENTER_DB'',''1'',''CLOUD'',?,?,''SNOWFLAKE'',
    ...
    var sf_stmt = snowflake.createStatement({sqlText: v_stmt, binds : [TABLE_NAME,LOAD_TYPE, SCD_TYPE]});
...
catch(err) 
{
	...
	var ins_fail_stmt = `update ETL.DW_CONTROL_TABLE set job_status =''FAILURE'' , error_message = ? 
	where target_name = ?;`
	var sf_ins_fail_stmt = snowflake.createStatement({sqlText: ins_fail_stmt,binds : [err.message,TABLE_NAME]});
